...
pub struct RecentOperations {
    ...
    /// How many events the ring buffer is holding, which `limit` may cut into.
    ///
    /// Bounded by `RECENT_EVENT_CAP` rather than by history: this is what the
    /// registry still remembers, not everything that ever ran.
    pub total: usize,
}
...
impl OperationRegistry {
    ...
    ///
    /// `limit` arrives as the wire type and is clamped rather than parsed: the
    /// clamp makes the conversion total, so this has no failure mode and its
    /// caller has no error arm to write.
    pub fn recent(&self, limit: Option<i64>) -> RecentOperations {
        let inner = self.lock_inner();
        ...
        let limit = vibrev_kit::page::capped(limit, RECENT_EVENT_CAP, MAX_RECENT_LIMIT);

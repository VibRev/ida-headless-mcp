...
pub fn handle_list_globals(idb: &Option<IDB>, query: &NameQuery) -> Result<Value, ToolError> {
    ...
    for name in db.names().iter() {
        ...
        total += 1;
        if !full_scan && (total <= query.offset || globals.len() >= query.limit) {
            continue;
        }
    ...
    query.sort(&mut globals, |g| &g.address, |g| &g.name);

    let next_offset = page::next_offset(query.offset, globals.len(), total);
...
pub fn handle_idb_meta(idb: &Option<IDB>) -> Result<Value, ToolError> {
    ...
    let input_file_path = input_file_path.trim_end_matches('\0');

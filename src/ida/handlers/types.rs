...
pub fn handle_local_types(
    ...
    for ordinal in 1..ordinal_limit {
        ...
        if !name_filter.matches(&info.name) || !query.kind_matches(&info.kind) {
            continue;
        }

        total += 1;
        if !full_scan && (total <= query.offset || types.len() >= query.limit) {
            continue;
        }
    ...
    query.sort(&mut types, |t| t.ordinal, |t| &t.name);

    let next_offset = page::next_offset(query.offset, types.len(), total);

...
use crate::ida::types::{DbInfo, DebugInfoLoad, OpenSpec};
...
/// Sibling `.i64` path for a raw binary: append the suffix, do not replace
/// an existing extension. `kernelcache.release.iphone` must become
/// `kernelcache.release.iphone.i64`, not `kernelcache.release.i64`.
pub fn idb_path_for_raw_binary(path: &Path) -> PathBuf {
...
pub fn handle_open(
    ...
    let load_debug_info = spec.load_debug_info;
    let debug_info_verbose = spec.debug_info_verbose;
    let force = spec.force;
    let rebuild = spec.rebuild;
    let auto_analyse = spec.auto_analyse;
    let extra_args = spec.extra_args.as_slice();
    let idb_out = spec.idb_out.as_deref();
    let expanded = expand_path(path);
    let debug_info_path = non_empty_trimmed(spec.debug_info_path.as_deref());
    let file_type = non_empty_trimmed(spec.file_type.as_deref());

...
/// Cyclomatic complexity `E - N + 2` over a function's basic-block graph.
///
/// `N` is the block count and `E` is the number of successor edges. A
/// function with no blocks is 0 rather than the `2` the formula would give
/// for an empty graph: there is no CFG to be complex.
pub(crate) fn cyclomatic_complexity(node_count: usize, edge_count: usize) -> usize {
    if node_count == 0 {
        0
    } else {
        edge_count.saturating_add(2).saturating_sub(node_count)
    }
}
